...
export const postTellerApi = async (teller: {username: string; password: string; fullName: string; email: string; phone: string;}) => {
   return (await instance.post("/admin/tellers", teller)).data;
}

export const updateTellerApi = async (tellerId: number, teller: {username?: string; password?: string; fullName?: string; email?: string; phone?: string;}) => {
