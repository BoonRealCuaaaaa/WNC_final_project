...
import { createBeneficiaryApi, editBeneficiaryApi } from "@/api/beneficiaries.api"
...
export default function EditBeneficiaryForm({beneficiary, onSuccess}: Props) {
    // 1. Define your form.
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            ...
            remindName: beneficiary.remindName,
        },
    })
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        mutateCreateBeneficiary({id: beneficiary.id, bankName, accountNumber, remindName });
    }
    ...
    const { mutate: mutateCreateBeneficiary } = useMutation({
        mutationFn: editBeneficiaryApi,
        onSuccess: (response) => {
            if (response.status === 200) {
                setSubmitStatus("success");
                toast({
                    variant: "default",
                    title: "Chỉnh sửa người thụ hưởng thành công"
                });
            }
        },
        onError: (error: AxiosError) => {
            ...
            toast({
                variant: "destructive",
                title: "Chỉnh sửa người thụ hưởng thất bại",
                description: message,
            });
        },
    });
